...
#File_name = 'H:\\Documents\\HstrSteel\\Excel_files\\20190516_PresentData.xlsx'
File_name = 'C:\\Users\\gabsab\\Documents\\Personal\\Excel_files\\20190516_PresentData.xlsx'
...
dbase_a = pd.read_excel(File_name, 'Curve_a', header=0)
########################################################################################################################
#pyplot
...
def regression_models(database, name, save_dir):
    ...
    sns.set_style("whitegrid",
                  rc={'axes.grid': True, 'grid.linestyle': '-', 'grid.color': 'xkcd:light grey', 'xtick.bottom': True,
                      ...
                      'font.size': 10, 'font': 'calibri', 'pdf.fonttype': 42})
    ...
    distribution = (sns.distplot(database[name].dropna(), ax=ax[1], axlabel=False, vertical=True, rug=False,
                                 color='black',
                                 bins=np.arange(0.6, math.ceil(np.max(database[name].dropna())*10)/10 + 0.2, 0.1),
                                 kde=True, kde_kws={"linestyle": "--", "linewidth": 0.5, "zorder": 3, "bw": 'silverman'}))
    ...
    if lower_90 >= 0.75:
        plt.text(xmax, lower_90, ci_text, color='black', horizontalalignment='right', verticalalignment='bottom',
                 fontsize=10, rotation='horizontal')
    higher_90 = res_mean + 1.64 * res_std
    ci_text = "%.3f " % higher_90
    if higher_90 <= np.max(database[name]):
        plt.text(xmax, higher_90, ci_text, color='black', horizontalalignment='right', verticalalignment='bottom',
...
def regression_models2(database, name, fabrication, save_dir, y_interval=None):
    inch = 1
    cm = inch / 2.54
...
def regression_models_red(database, name, fabrication, section_type, save_dir, y_interval=None):
    ...
    s = database.groupby("Steel Grade").size()
    grades = s.index[s > 0]
    R_data = database.loc[database.Author == 'Sabau']
    V_data = database.loc[database.Author != 'Sabau']
    for steel_grade in grades:
        cond_markers = ['s' if steel_grade == 'S960' else '^' if steel_grade == 'S700' else 'o']
        sc = ax[0].scatter(x="Slenderness", y=name, data=V_data[V_data['Steel Grade'] == steel_grade],
                           marker=cond_markers[0], label=steel_grade, c='black', zorder=8)
        if cond_markers == ['o']:
            sc.set(facecolors='none', edgecolors='black')
    if R_data.empty is False:
        sc = ax[0].scatter(x="Slenderness", y=name, data=R_data,
                           marker='s', label='SX960', c='red', zorder=8)
        sc.set(facecolors='red', edgecolors='black')
    ...
    # plt.savefig(fig_title, format='jpg', dpi=1000)  # bbox_inches='tight'
    # fig_title = save_dir + name + fabrication + section_type + ".eps"
    # plt.savefig(fig_title, format='eps', dpi=1000)  # bbox_inches='tight'
    #plt.show()
    #plt.close(fig='all')
...
b_curve = "C:\\Documents\\HstrSteel\\Figures\\curve_b\\"
c_curve = "C:\\Documents\\HstrSteel\\Figures\\curve_c\\"
a_curve = "C:\\Documents\\HstrSteel\\Figures\\curve_a\\"
c_curve_exp = 'C:\\Users\\gabsab\\Documents\\Personal\\Thesis_related\\Statistical_figures\\curve_c_exp\\'
sk = "C:\\Documents\\HstrSteel\\Figures\\SK\\"
main = "C:\\Documents\\HstrSteel\\Figures\\"
main_exp = 'C:\\Users\\gabsab\\Documents\\Personal\\Thesis_related\\Statistical_figures\\main_exp\\'

i = "H:\\Documents\\HstrSteel\\Figures\\i\\"
b = "H:\\Documents\\HstrSteel\\Figures\\b\\"
...
# #comparison of different facbrication methods
# #RHS
# regression_models_red(dbase, "JSvsT", "Cold-formed", "RHS", c_curve, y_interval=[0.8, 1.6])
# regression_models_red(dbase, "ENvsT", "Cold-formed", "RHS", c_curve, y_interval=[0.8, 2.2])
# regression_models_red(dbase, "AISCvsT", "Cold-formed", "RHS", main, y_interval=[0.8, 2.2])
# #
# regression_models_red(dbase, "JSvsT", "Seamless", "RHS", main, y_interval=[0.8, 1.6])
# regression_models_red(dbase, "ENvsT", "Seamless", "RHS", main)
# regression_models_red(dbase, "AISCvsT", "Seamless", "RHS", main)
# #
# regression_models_red(dbase, "ENvsT", "Welded", "RHS", c_curve, y_interval=[0.8, 2.2])
# regression_models_red(dbase, "JSvsT", "Welded", "RHS", c_curve, y_interval=[0.8, 2.2])
# regression_models_red(dbase, "SKvsT", "Welded", "RHS", sk, y_interval=[0.8, 2.2])
# regression_models_red(dbase, "SKKSvsT", "Welded", "RHS", sk, y_interval=[0.8, 2.2])
# regression_models_red(dbase, "SKvsT_lim", "Welded", "RHS", sk, y_interval=[0.8, 2.2])
# regression_models_red(dbase, "AISCvsT", "Welded", "RHS", main, y_interval=[0.8, 2.2])
#
#regression_models_red(dbase_b, "ENvsT", "Welded", "RHS", b_curve)
#regression_models_red(dbase_b, "JSvsT", "Welded", "RHS", b_curve)
#
# #I-sections
regression_models_red(dbase, "JSvsT", "Welded", "I-sections", c_curve_exp, y_interval=[0.8, 1.9])
regression_models_red(dbase, "ENvsT", "Welded", "I-sections", c_curve_exp, y_interval=[0.8, 1.9])
regression_models_red(dbase, "SKvsT", "Welded", "I-sections", c_curve_exp, y_interval=[0.8, 1.9])
regression_models_red(dbase, "AISCvsT", "Welded", "I-sections", main_exp, y_interval=[0.8, 1.9])
regression_models_red(dbase, "ModelvsT", "Welded", "I-sections", c_curve_exp, y_interval=[0.8, 1.9])
regression_models_red(dbase, "ModelvsT", "Welded", "RHS", c_curve_exp, y_interval=[0.8, 1.9])
regression_models_red(dbase, "Model1000vsT", "Welded", "I-sections", c_curve_exp, y_interval=[0.8, 1.9])
regression_models_red(dbase, "Model1000vsT", "Welded", "RHS", c_curve_exp, y_interval=[0.8, 1.9])
#
#
