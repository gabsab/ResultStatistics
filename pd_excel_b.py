...
def regression_models(database, name, save_dir):
    ...
    fig, ax = plt.subplots(nrows=1, ncols=2,  figsize=[17*cm, 6*cm], constrained_layout=True,
    ...
    sns.set_style("whitegrid",
                  rc={'axes.grid': True, 'grid.linestyle': '-', 'grid.color': 'xkcd:light grey', 'xtick.bottom': True,
                      'ytick.left': True, 'marker.size': '0.5', "xtick.direction": "out", "ytick.direction": "out",
    ...
    ax[0].plot([0, 2.5], [1, 1], linestyle='-', color='white', linewidth=2, zorder=2)
    regression = (sns.regplot(x_ci=90, x="Slenderness", y=name, x_bins=np.arange(0, 2.4, 0.2), x_estimator=np.mean,
                              data=database, ax=ax[0], color='white', fit_reg=True, truncate=True, marker="_",
    ...
    ax[1].plot(fit_pdf, lnspc, lw=1, color='white', label='Norm PDF', zorder=4)
    ix = []
    iy = []
    ...
    plt.text(xmax, higher_90, ci_text, color='red', horizontalalignment='right', verticalalignment='bottom',
            fontsize=10, rotation='horizontal', zorder=10)
    ax[1].plot([xmin, xmax+1], [lower_90, lower_90], '--', color='white', linewidth=1, markersize=5, zorder=10)
    ax[1].plot([xmin, xmax+1], [higher_90, higher_90], '--', color='white', linewidth=1, markersize=5, zorder=10)
    ax[1].plot([xmin, xmax+1], [1, 1], '-', color='white', linewidth=2, zorder=2)
    ...
    h = ax[0].text(0.0120, 0.97, str(info_text), color='white', horizontalalignment='left',
    ...
    fig_title = save_dir + name+"_b.png"
    plt.savefig(fig_title,  format='png', dpi=1000) #bbox_inches='tight',
    fig_title = save_dir + name + "_b.pdf"
...
def regression_models2(database, name, fabrication, save_dir):
    ...
    fig, ax = plt.subplots(nrows=1, ncols=2,  figsize=[17*cm, 5*cm], constrained_layout=True,
                           gridspec_kw={"wspace": 0.05, 'width_ratios': [5, 1]}, )
    sns.set_context("paper")
    ...
    h = ax[0].text(0.0120, 0.97, str(info_text), color='white', horizontalalignment='left',
                   verticalalignment='top', fontsize=10, rotation='horizontal', zorder=10, transform=ax[0].transAxes,
                   bbox={'facecolor': 'black', 'edgecolor': 'white'}) # 'pad':10
    fig_title = save_dir + name + fabrication + "_b.png"
    plt.savefig(fig_title, format='png', dpi=1000) #bbox_inches='tight'
    fig_title = save_dir + name + fabrication + "_b.pdf"
    plt.savefig(fig_title, format='pdf', dpi=1000) #bbox_inches='tight'
    fig_title = save_dir + name + fabrication + "_b.jpg"
...
def regression_models_red(database, name, fabrication, section_type, save_dir):
    ...
    for steel_grade in grades:
        ...
        sc = ax[0].scatter(x="Slenderness", y=name, data=database[database['Steel Grade'] == steel_grade],
                      marker=cond_markers[0], label=steel_grade, c='white', zorder=8)
        if cond_markers == ['o']:
            sc.set(facecolors='none', edgecolors='white')

    label_list = ax[0].get_legend_handles_labels()[1]
    label_list.reverse()
    handles_list = ax[0].get_legend_handles_labels()[0]
    handles_list.reverse()
    ax[0].legend(handles_list, label_list)
    ...
    y_lim = y_lim if y_lim > 1.9 else 1.9
    xmin, xmax = ax[1].get_xlim()
    ax[1].set_xlim([xmin, math.ceil(xmax)+0.01])
    ...
    fig_title = save_dir + name + fabrication + section_type + "_b.png"
    plt.savefig(fig_title, format='png', dpi=1000) #bbox_inches='tight'
    fig_title = save_dir + name + fabrication + section_type + "_b.pdf"
    plt.savefig(fig_title, format='pdf', dpi=1000) #bbox_inches='tight'
    fig_title = save_dir + name + fabrication + section_type + "_b.jpg"
    plt.savefig(fig_title, format='jpg', dpi=1000)  # bbox_inches='tight'
    #plt.show()
    plt.close(fig='all')
...
# # Jönsson & Stan vs experimental results
#regression_models2(dbase_b, "JSvsT", "Welded", b_curve)
#regression_models2(dbase_b, "ENvsT", "Welded", b_curve)

#comparison of different facbrication methods
#RHS
#regression_models_red(dbase, "JSvsT", "Cold-formed", "RHS", c_curve)
# regression_models_red(dbase, "JSvsT", "Welded", "RHS", c_curve)
